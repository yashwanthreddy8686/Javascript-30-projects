const inventors = [
  ...
  const firstNameAndLastName = inventors.map(inventor => `${inventor.first}, ${inventor.last}`);
  ...
  const sortedByNoOfYearsLived = inventors.sort((a,b) => (a.passed - a.year) > (b.passed - b.year) ? -1: 1);
  console.table(sortedByNoOfYearsLived);
